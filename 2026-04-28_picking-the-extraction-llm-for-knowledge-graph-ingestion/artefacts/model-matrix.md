...
## Partial, aborted, or incomplete local runs (not ranked)

These rows are not comparable to the headline matrix because ingest did not finish on all 20 smoke documents (or aborted before meaningful graph output). Smoke scores and density columns are shown for diagnostic context only.

| Model | Provider / runtime | Ent / doc | Rel / doc | Rel / ent | Smoke (pass / marg / fail) | Notes |
|---|---|---:|---:|---:|---|---|
| Qwen 2.5 14B GGUF Q4_K_M | LM Studio llama.cpp, M1 Max (32 GB) | 21.9 | 19.7 | 0.90 | 5 / 4 / 15 | Best M1 Max completion; **19/20 documents** in ~4 hours. Densities and smoke score are not cross-comparable to full-corpus rows. |
| Gemma 4 26B Q4 | Ollama, consumer GPU (6 GB VRAM) | aborted | aborted | aborted | timeout on chunk 1 | Thinking-mode tokens push every call past worker timeout. |
| Qwen 2.5 32B Q4 | Ollama, consumer GPU | partial | partial | partial | HTTP 502 cascade | Out of VRAM, repeated load/unload cascade. |
| Qwen 2.5 14B 4-bit MLX | LM Studio MLX, M1 Max | partial | partial | partial | embed cascade abort | MLX backend interacted poorly with concurrent embedder calls. |
...
The April matrix above was run with all commercial rows ingested via OpenRouter. The current production stack routes Gemini 2.5 Flash through an enterprise gateway over a private network for the primary working workspace, and still uses OpenRouter for the legal-corpus workspace. The pricing ratios in the cost-model artefact derive from public list prices and survive the routing change; the per-call latency and the per-day quota shape differ between OpenRouter and the enterprise gateway and would warrant their own measurement on whichever path you deploy.

## Sources

All commercial rows ingested via OpenRouter at run time (April 2026). All local rows ingested via Ollama or LM Studio on the test hosts: an Apple Silicon M1 Max workstation (32 GB unified memory) and a consumer-tier x86 workstation with a GTX 1070 (6 GB VRAM). Raw smoke transcripts and ingest logs are held in an internal investigation log; specific filenames are not published because the surrounding corpus contains non-public material. The 17-document partial-ingest data point comes from a working LightRAG instance on the same test deployment, exported at a checkpoint.
